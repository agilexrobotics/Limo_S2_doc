...
松灵机器人Limo_S2是一款集全向轮与机械臂一体的ROS开发平台，提供了适应场景更广泛、更符合行业应用要求的学习平台，适用于机器人教育、功能研发、产品开发。可在配套的专业沙盘中快速建立多场景实拟教学和测试，LIMO_S2搭载NVIDIA Jeston Nano、激光雷达、深度相机等高性能传感器配置，可实现精确的自主定位、SLAM建图、路线规划和自主避障、红绿灯识别等丰富功能。

同时，松灵机器人联合国内ROS社区教学开创者古月居，致力于结合企业用人和行业应用需求，打造基于Limo_S2开发小车的全新ROS精品课程，助力院校科研教学，让学生达到更高的行业应用技术要求。
...
![](LIMO_S2_image/等轴侧.svg)

![](LIMO_S2_image/等轴侧反向.svg)
...
![img](./LIMO_S2_image/App_4.png)

![img](./LIMO_S2_image/App_5.png)
...
首先在个人电脑下载相应的软件，下载链接：https://www.nomachine.com/download，根据自己电脑的操作系统和架构下载相应的版本。让Limo_S2和电脑连接到同一个WIFI下。                         

#### 1.9.2 连接wifi

Limo_S2的后端有USB借口和HDMI接口，需要接上显示器之后才能连接wifi；然后给Limo_S2连接上键盘鼠标；键盘鼠标成功连接之后通过以下操作连接wifi，选择需要连接的wifi。

![](./LIMO_S2_image/wifi_1.png)

输入wifi的密码

![](./LIMO_S2_image/wifi_2.png)
...
![](./LIMO_S2_image/Nomachine.png)

点击Yes

![](./LIMO_S2_image/Nomachine_2.png)

Username：agilex   Password：agx 勾选保存密码

![](./LIMO_S2_image/Nomachine_3.png)

一路选择默认OK

![](./LIMO_S2_image/Nomachine_4.png)
...
#### 2.1.1 电池基本信息
 ...
 Limo_S2随车配备一个12V的电池，该电池有两个接口。分别为黄色的电池输出接口和黑色的电池充电接口。
...
- 为了保证运输存储安全，Limo_S2配备的电池并不一定处于满电状态 。
- 请不要在电池使用殆尽以后再进行充电，当 Limo_S2提示电量低时，请及时充电。
- Limo_S2在关机状态下仍会产生静态待机电流，为防止电池过放，长时间不使用Limo_S2时请断开电池与车体的连接。
- 请勿将电池投火中，或对电池加热，请勿在高温下存储电池，电池存储的最佳温度为-10℃~40℃。
- 必须使用原厂配备或认证的电池为Limo_S2供电。

------

####  2.1.2 充电

Limo_S2默认随车配备一个12.6V 2A的充电器，可满足客户的充电需求，且充电器上设有指示灯可显示充电状态。
...
- 充电时必须将电池与Limo_S2车体分离，禁止在电池充电的同时为Limo_S2进行供电。
...
- Limo_S2的工作温度为-10℃ ~ 40℃，请勿在温度低于-10℃、高于40℃环境中使用；
- Limo_S2的使用环境的相对湿度要求是：最大80%，最小30%；
- 请勿在存在腐蚀性、易燃性气体的环境或者靠近可燃性物质的环境中使用；
- Limo_S2不具有防水功能，请勿在有雨、雪、积水的环境使用；
...
![](./LIMO_S2_image/电源拓扑.png)

------

- Limo_S2的电池能提供最大10A的电流给底盘、Nano及传感器等系统供电，当系统检测到工作电流大于10A时，为了保护电池和电机会进入过流保护模式。
...
- Limo_S2底盘内置了蓝牙5.0模块可以与手机端的APP连接，实现遥控功能。
- Limo_S2与Nano通过UART接口直接连接，Nano通过该接口可实现对底盘的控制。
...
移动底盘需要通过程序驱动才能实现limo的导航，Limo_S2的底盘驱动程序分为两个版本，分别为C++版本和Python版本，两个版本都可以控制Limo_S2运动。
...
在终端输入命令之后，Limo_S2会先前行走一小段距离，然后停下。
...
![](./LIMO_S2_image/lidar.png)

### 4.2 gmapping建图

#### 4.2.1 gmapping建图算法介绍

Gmapping是基于滤波SLAM框架的常用开源SLAM算法。Gmapping有效利用了车轮里程计信息，对激光雷达的频率要求不高，在构建小场景地图时，所需的计算量较小且精度较高。这里通过使用ROS封装了的GMapping功能包来实现limo_s2的建图。

#### 4.2.2 gmapping建图实践操作

注：在运行命令之前，请确保其他终端中的程序已经终止，终止命令为：Ctrl+c

注：建图过程中Limo_S2的速度尽量慢点，速度太快会影响建图的效果
...
![](./LIMO_S2_image/gmapping.png)

这时候就可以把手柄调为遥控模式，控制Limo_S2建图了。
...
![](./LIMO_S2_image/navi_diff.png)

（3）开启导航之后，会发现激光扫描出来的形状和地图没有重合，需要我们手动校正，在rviz中显示的地图上矫正底盘在场景中实际的位置，通过rviz中的工具，发布一个大概的位置，给Limo_S2一个大致的位置，然后通过手柄遥控Limo_S2旋转，让其自动校正，当激光形状和地图中的场景形状重叠的时候，校正完成。操作步骤如图 ：

![](./LIMO_S2_image/limo_tu_02.png)

校正完成后

![](./LIMO_S2_image/navi3.png)

（4）通过2D Nav Goal 设置导航目标点。

![](./LIMO_S2_image/limo_tu_03.png)

地图中将会生成一条紫色的路径，手机切换至指令模式，Limo_S2将自动导航到目标点

![](./LIMO_S2_image/navi_5.png)
...
![](./LIMO_S2_image/traffic_light.png)
...
<img src="./LIMO_S2_image/正视.svg"  width="800"  height = "600" />

<img src="./LIMO_S2_image/左视.svg"  width="800"  height = "600" />

<img src="./LIMO_S2_image/上视.svg"  width="800"  height = "600" />
...
使用话题通信机制控制Limo_S2运动，需要编写Publisher和Subscriber两个节点，Publisher负责向Limo_S2发布速度控制指令，Subscriber负责监听Limo_S2接收到的速度大小。

##### 4.3.1 如何创建Publisher

Publisher的主要作用是针对指定话题发布特定数据类型的消息。我们尝试使用代码实现一个节点，节点中创建一个Publisher并控制Limo_S2运动。源码文件位置为~/agilex_ws/src/limo_ros/learning_limo/src/talker.cpp，其内容如下：
...
在Limo_S2中，环境变量已经设置好了，可以按照以下步骤启动例程：
...
![](./LIMO_S2_image/tf_tree.png)



### 附录5、系统烧录

Limo_S2有两个版本的nano，分别为SD卡版本和emmc版本，两个版本的烧录方式不一样，烧录的时候注意区分。
...
![](./LIMO_S2_image/balena_1.png)

![](./LIMO_S2_image/balena_2.png)



2、选择需要进行烧录的SD卡

![](./LIMO_S2_image/balena_4.png)

3、点击Flash，开始烧录

![](./LIMO_S2_image/balena_6.png)
...
​	首先，需要把Limo_S2上的nano拆卸下来，拆的过程中需要注意不要损坏零件。
...
![](./LIMO_S2_image/emmc_3.png)

当system.img开始写入时，系统开始烧录了。

![](./LIMO_S2_image/emmc_4.png)

当出现successful时，系统就烧录完成了。

### 附录 6、固件升级

固件升级的软件在Limo_S2的主目录下，文件名称为LimonTest_Nano，升级所需要的固件在我司官方GitHub中，链接为：https://github.com/agilexrobotics/limo-doc

注：如Limo_S2中的LimonTest_Nano不能正常打开，请到我司官方GitHub中下载最新的软件，链接为：https://github.com/agilexrobotics/limo-doc
